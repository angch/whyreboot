...
fn correlate(findings: &mut [Finding]) {
    ...
    for i in 0..n {
        match findings[i].category.as_str() {
            "GPU" => {
                for j in 0..n {
                    if i == j || !near(&findings[i], &findings[j]) {
                        continue;
                    }
                    if matches!(
                        findings[j].category.as_str(),
...
fn is_kernel(line: &LogLine) -> bool {
    line.transport.eq_ignore_ascii_case("kernel") || line.identifier.eq_ignore_ascii_case("kernel")
}
...
fn detect_kernel_panic(line: &LogLine) -> Option<Finding> {
    ...
    let title = if contains_ci(&line.message, "userspace watchdog timeout") {
        ...
        let who = line
            .message
            .find("no successful checkins from")
            .map(|p| {
                line.message[p + 27..]
                    .trim()
                    .split([' ', '(', '\n'])
    ...
    Some(finding(
        ...
        title,
        vec![
            "The kernel hit an unrecoverable fault. If the machine rebooted, this is \
              almost certainly why. Linux: `journalctl -k -b -1`; macOS: check \
              /Library/Logs/DiagnosticReports for the matching .panic file."
                .into(),
        ],
        "kernel",
    ))
...
fn detect_shutdown_cause(line: &LogLine) -> Option<Finding> {
    ...
    let (sev, meaning) = match code {
        ...
        0 => (
            Severity::Critical,
            "power loss — the Mac lost power without shutting down",
        ),
        3 => (Severity::Warning, "hard shutdown — power button held down"),
        -3 => (
            Severity::Critical,
            "multiple temperature sensors exceeded limits (thermal)",
        ),
        -60 => (
            Severity::Critical,
            "bad master directory block — serious disk corruption",
        ),
        -61 | -62 => (
            Severity::Critical,
        ...
        -78 | -79 => (
            Severity::Critical,
            "incorrect current from power adapter/battery",
        ),
        ...
        -128 => (
            Severity::Warning,
            "unknown cause — often follows a panic or forced reset",
        ),
        _ => (
            Severity::Warning,
            "unlisted cause code — non-clean shutdown suspected",
        ),
    };
    ...
    Some(finding(
        ...
        vec![
            "macOS logs this at the boot AFTER the event — it describes how the \
              PREVIOUS shutdown ended."
                .into(),
            "Recurring thermal/battery codes point at hardware; watchdog codes \
              (-61/-62) point at a hung process, often graphics-related."
                .into(),
        ],
        "macos:kernel",
    ))
...
fn detect_gpu(line: &LogLine) -> Option<Finding> {
    ...
    if m.starts_with("ring")
        && !(contains_ci(msg, "timeout")
            || contains_ci(msg, "timed out")
            || contains_ci(msg, "error")
            || contains_ci(msg, "hang")
            || contains_ci(msg, "fail")
            || contains_ci(msg, "reset"))
    {
        return None;
    }
    // "legacy queue" only signals a fault when the operation on it failed.
    if m == "legacy queue" && !contains_ci(msg, "fail") {
        return None;
    }

    let driver = ["amdgpu", "i915", "nouveau", "radeon"]
        .iter()
        .find(|d| contains_ci(msg, d))
        .copied()
    ...
    if m == "*ERROR*" && !msg.contains("[drm") && driver == "drm" {
        return None;
    }
    ...
    let culprit = msg
        ...
        .or_else(|| {
            msg.trim().strip_prefix("Process ").and_then(|rest| {
                rest.contains(" pid ")
                    .then(|| rest[..rest.find(" pid ").unwrap()].trim().to_string())
            })
        })
        .filter(|c| !c.is_empty());

    let soft = contains_ci(msg, "soft recovered")
        || contains_ci(msg, "flip_done")
        || is_app_level_xid(msg);
    let (sev, what) = if contains_ci(msg, "fallen off the bus") {
        (
            Severity::Critical,
            "GPU has fallen off the bus (device unreachable)".to_string(),
        )
    } else if soft {
        (
            Severity::Warning,
            format!("GPU fault ({driver}) — recovered/soft"),
        )
    } else {
    ...
    evidence.push(match sev {
        ...
        _ => "The GPU driver recovered from a fault without a full reset. Occasional \
...
fn detect_segfault(line: &LogLine) -> Option<Finding> {
    ...
    let who = comm_pid_prefix(&line.message)
        .map(|(c, p)| match p {
            Some(p) => format!("{c} (pid {p})"),
            None => c,
        })
        .unwrap_or_else(|| "a process".to_string());
    let kind = if m == "segfault at" {
        "segmentation fault"
    } else if m.starts_with("general") {
        "general protection fault"
    } else {
        "an illegal-instruction / trap fault"
    };
    Some(finding(
        line,
        Severity::Warning,
        ...
        vec![
            "A process died on a memory-access or instruction fault — likely a bug or \
              corrupted binary. Check for a matching coredump."
                .into(),
        ],
        "journald:kernel",
    ))
...
fn detect_thermal(line: &LogLine) -> Option<Finding> {
    ...
    let (sev, title) = if critical {
        (
            Severity::Critical,
            "Critical temperature reached — thermal shutdown risk",
        )
    } else {
        (
            Severity::Warning,
    ...
    Some(finding(
        ...
        title.to_string(),
        vec![
            "The CPU/system got hot enough to throttle or trip. Check cooling, fans, dust, \
              and airflow; sustained throttling degrades performance."
                .into(),
        ],
        "journald:kernel",
    ))
...
fn detect_hardware(line: &LogLine) -> Option<Finding> {
    ...
    if m == "acronym" {
        let errorish = contains_ci(msg, "error")
            || contains_ci(msg, "fail")
            || msg.contains(" CE ")
            || msg.contains(" UE ");
        if !errorish {
    ...
    Some(finding(
        ...
        title.to_string(),
        vec![
            "The platform reported a hardware fault (CPU/memory/PCIe). Persistent errors \
...
fn detect_coredump(line: &LogLine) -> Option<Finding> {
    ...
    if !by_id && !contains_ci(&line.message, "dumped core") {
        return None;
    }
    if is_kernel(line) {
        return None;
    } // kernel "dumped core" duplicates segfault handling
    // "Process 1234 (comm) of user 1000 dumped core."
    let comm = line.message.find('(').and_then(|a| {
        line.message[a + 1..]
            .find(')')
            .map(|b| line.message[a + 1..a + 1 + b].to_string())
    });
    let who = comm
        .map(|c| format!("'{c}'"))
        .unwrap_or_else(|| "a process".to_string());
    Some(finding(
        ...
        vec![
            "A crash was captured. Inspect the backtrace with `coredumpctl info` / \
              `coredumpctl gdb`."
                .into(),
        ],
        "systemd-coredump",
...
fn detect_session(line: &LogLine) -> Option<Finding> {
            ...
            || msg.contains("Caught signal"))
    {
        return Some(finding(
            ...
            vec![
                "The X display server hit a fatal error — the whole graphical session \
                  died with it. Check /var/log/Xorg.0.log.old and the GPU driver; the \
                  (EE) Backtrace lines name the faulting module."
                    .into(),
            ],
            "journald:x11",
        ));
        ...
        && contains_ci(msg, "Unrecoverable failure in required component")
    {
        ...
        return Some(finding(
            ...
            vec![
                "A core part of the GNOME session (often the gnome-shell compositor) \
                  failed. Look for a matching coredump/segfault just before this."
                    .into(),
            ],
            "gnome-session",
        ));
    ...
    if first_of(msg, &lost).is_some() {
        let who = if line.identifier.is_empty() {
            "An app".to_string()
        } else {
        ...
        return Some(finding(
            ...
            vec![
                "The Wayland compositor (gnome-shell / kwin_wayland / …) went away — \
                  usually because it crashed, killing every client attached to it. \
                  Check for a compositor coredump/segfault at the same moment."
                    .into(),
            ],
            "journald:wayland",
        ));
...
fn detect_mac_crash_report(line: &LogLine) -> Option<Finding> {
    if !line.identifier.eq_ignore_ascii_case("ReportCrash") {
        return None;
    }
    let msg = &line.message;
    ...
    Some(finding(
        ...
        vec![
            "ReportCrash captured a crash report. Open the .ips file in Console.app \
              (Crash Reports) for the backtrace."
                .into(),
        ],
        "macos:ReportCrash",
    ))
...
fn detect_mac_sleep_wake(line: &LogLine) -> Option<Finding> {
    if !contains_ci(&line.message, "Sleep Wake failure") {
        return None;
    }
    Some(finding(
        line,
        ...
        vec![
            "Often driver/peripheral related: disconnect USB/Thunderbolt devices, \
              reset SMC/NVRAM on Intel Macs, and check for a matching Sleep Wake \
              Failure report in Console.app."
                .into(),
        ],
        "macos:kernel",
    ))
...
fn detect_mac_update_restart(line: &LogLine) -> Option<Finding> {
    ...
    Some(finding(
        ...
        vec![
            "A macOS software update armed or performed a restart — an expected \
              reboot, listed so unexplained reboots can be told apart from updates."
                .into(),
        ],
...
mod tests {
    ...
    fn mce_uncorrected_is_critical() {
        let f = classify(&kline("mce: [Hardware Error]: Machine check events logged")).unwrap();
    ...
    fn macos_redacted_tcp_summary_is_not_a_hardware_error() {
        ...
        assert!(
            classify(&kline(msg)).is_none(),
            "redacted tcp summary must not match"
        );
    ...
    fn service_failure_needs_systemd_identifier() {
        ...
        assert!(
            classify(&kline("nginx.service: Failed with result 'exit-code'.")).is_none()
                || classify(&kline("nginx.service: Failed with result 'exit-code'."))
                    .unwrap()
                    .category
                    != "Service"
        );
    ...
    fn amdgpu_reset_and_vram_lost_detected() {
        assert_eq!(
            classify(&kline("amdgpu 0000:0c:00.0: amdgpu: GPU reset begin!"))
                .unwrap()
                .category,
            "GPU"
        );
        assert_eq!(
            classify(&kline("[drm] VRAM is lost due to GPU reset!"))
                .unwrap()
                .category,
            "GPU"
        );
    ...
    fn amdgpu_soft_recovery_is_warning() {
        let f = classify(&kline("[drm] ring gfx_0.0.0 timeout, but soft recovered")).unwrap();
        assert_eq!(f.category, "GPU");
    ...
    fn i915_gpu_hang_extracts_culprit() {
        ...
        assert_eq!(f.severity, Severity::Critical);
        assert!(
            f.evidence.iter().any(|e| e.contains("kwin_wayland")),
            "{:?}",
            f.evidence
        );
    }
    ...
    fn strix_halo_mes_ring_reset_burst_is_one_gpu_finding() {
        ...
        for msg in burst {
            ...
            assert_eq!(
                f.as_ref().map(|f| f.category.as_str()),
                Some("GPU"),
                "line not detected as GPU: {msg}"
            );
        }
        let lines: Vec<LogLine> = burst
            .iter()
            .enumerate()
            .map(|(i, m)| kline_at(base + i as i64, m))
            .collect();
    ...
    fn non_drm_error_convention_not_gpu() {
        ...
        assert!(classify(&kline("some_subsys: *ERROR* widget calibration failed")).is_none());
    }
    ...
    fn shutdown_cause_power_loss_and_hard_button() {
        assert_eq!(
            classify(&kline("Previous shutdown cause: 0"))
                .unwrap()
                .severity,
            Severity::Critical
        );
    ...
    fn mac_update_restart_needs_identifier() {
        let f = classify(&user_line(
            "softwareupdated",
            "SUOSUShimController: Armed: YES, with reason: install-and-restart",
        ))
        .unwrap();
    ...
    fn gpu_incident_correlates_with_following_crashes() {
        ...
        let lines = vec![
            kline_at(
                base,
                "[drm:amdgpu_job_timedout [amdgpu]] *ERROR* ring gfx_0.0.0 \
    ...
    fn ata_burst_coalesces_into_one_finding() {
        ...
        assert_eq!(
            found.len(),
            1,
            "the burst should collapse to a single Disk finding"
        );
        assert_eq!(found[0].category, "Disk");
        assert!(
            found[0].title.contains("related log lines"),
            "title: {}",
            found[0].title
        );
    }
