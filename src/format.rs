...
pub fn cause_detail(cause: &Cause) -> String {
    match cause {
        ...
        Cause::ForcedPowerOff => "Power button held down or power cable pulled".into(),
        ...
        Cause::WindowsUpdate {
            ...
            match (old_version, new_version) {
                ...
                (Some(_), Some(n)) => format!("{base} — running {}", win_product(n)),
                (None, Some(n)) => format!(
                    "{base} — now running {} (prior version not in log window)",
                    win_product(n)
                ),
                (Some(o), None) => format!("{base} — was running {}", win_product(o)),
                (None, None) => base,
            }
        }
        ...
        Cause::NormalShutdown => "System shut down cleanly; no specific initiator recorded".into(),
        Cause::Undetermined => "Insufficient event log data to determine cause".into(),
    }
}
...
fn explain_driver_power_failure(
    ...
    out.push(format!(
        "DRIVER_POWER_STATE_FAILURE: {} failed to complete a power state",
        m
    ));
    ...
    if p1 == 3 {
        out.push(String::new());
        out.push(format!(
            "P1=3: the OS sent an IRP_MN_SET_POWER request to {} but the driver",
            m
        ));
...
fn explain_win32k_power_watchdog(module: &Option<String>, out: &mut Vec<String>) {
    ...
    out.push(format!(
        "during a power transition. {} timed out waking the GPU from sleep.",
        m
    ));
    out.push(String::new());
    out.push("Recommended actions:".into());
...
mod tests {
    ...
    fn cause_label_all_variants() {
        ...
        assert!(
            cause_label(&Cause::WindowsUpdate {
                process: "x".into(),
                old_version: None,
                new_version: None
            })
        ...
        assert!(
            cause_label(&Cause::SystemProcess {
                process: "p".into(),
                reason: "r".into(),
                action: "a".into()
            })
    ...
    fn explanation_0x19c_mentions_gpu_or_display() {
        let cause = Cause::BlueScreen {
            stop_code: 0x19C,
            stop_name: "WIN32K_POWER_WATCHDOG_TIMEOUT",
            params: [0; 4],
        };
        let exp = generate_explanation(&cause, &Some("dxgkrnl".into()), &[]);
        assert!(!exp.is_empty());
        assert!(exp.iter().any(|l| l.contains("GPU")
            || l.to_lowercase().contains("display")
            || l.contains("watchdog")));
    }
